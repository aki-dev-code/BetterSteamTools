...
#include "BetterSteamToolsBuildInfo.h"
...
namespace {
    ...
    struct Snapshot {
        std::string BetterSteamToolsVersion = BETTERSTEAMTOOLS_VERSION;
    ...
    static std::string AppendSnapshot(std::string message)
    {
        message +=
            "\n\nSteam diagnostics:\n"
            "  BetterSteamTools version: " + g_snapshot.BetterSteamToolsVersion + "\n"
                ...
                const std::string& steamUIPath)
{
    ...
    LOG_INFO("SteamDiagnostics: ost.version={} build={} steamclient64.sha256={} steamui.sha256={}",
             g_snapshot.BetterSteamToolsVersion,
             g_snapshot.buildID,
