...
bool InitializeSteamComponents()
{
    ...
    sprintf_s(ConfigPath,      kRuntimePathCapacity, "%s\\bettersteamtools.toml", SteamInstallPath);
...
static uint32_t InitThread(OSTPlatform::DynamicLibrary::ModuleHandle selfModule) {
    Log::Init(selfModule);
    LOG_INFO("BetterSteamTools init thread started");
    ...
    LOG_INFO("BetterSteamTools init complete");
    return 0;
}
